...
function mostrarDatos(valorBuscar,pagina,cantidad,rut){
  path="http://mard.cl/nutricion/administrador/";
//path="http://localhost:8000/administrador/";
    $.ajax({
    url : path+"mostrar_usuarios_activados",
    type: "POST",
    ...
    success:function(response){
      ...
      $.each(response.usuarios,function(key,item){
        ...
        filas+="<tr><td><center>"+item.rut+"</td><td><center>"+item.Nombres+" "+item.Apellidos+"</td><td><center><a class='btn btn-danger tabla btn-xs' href='"+path+"desactivar_usuario/"+item.rut+"/')'"+"><span class='glyphicon glyphicon-remove' aria-hidden='true'></span></a></center></td></tr>";
      });
