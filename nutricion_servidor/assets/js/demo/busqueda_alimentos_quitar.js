...
//path="http://localhost:8000/preparacion/";
    $.ajax({
    url : path+"mostrar_alimentos_quitar/"+id_prep,
    type: "POST",
    ...
    success:function(response){
      ...
      $.each(response.alimento,function(key,item){
        filas+="<tr><td><center>"+item.id+"<center></td><td><center>"+item.nombre+"<center></td><td><center>"+item.tipo+"<center></td><td><center><a href='"+path+"quitar_alimentos/"+id_prep+"/"+item.idpa+ "'class='btn btn-danger tabla btn-xs'><span class='glyphicon glyphicon-remove' aria-hidden='true'></span></a></center></td></tr>";
      });
